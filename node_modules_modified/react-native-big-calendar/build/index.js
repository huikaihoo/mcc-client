...
var HourGuideColumn = React.memo(function (_a) {
    ...
    return (React.createElement(reactNative.View, { style: { height: cellHeight } },
        React.createElement(reactNative.Text, { style: commonStyles.guideText }, formatHour(hour, ampm))));
...
var CalendarBody = React.memo(function (_a) {
    ...
    return (React.createElement(reactNative.ScrollView, __assign({ style: [
        ...
        React.createElement(reactNative.View, __assign({ style: isRTL ? [styles$1.bodyRTL] : [styles$1.body] }, (reactNative.Platform.OS === 'web' ? panResponder.panHandlers : {})),
            React.createElement(reactNative.View, { style: [commonStyles.hourGuide] }, hours.map(function (hour) { return (React.createElement(HourGuideColumn, { key: hour, cellHeight: cellHeight, hour: hour, ampm: ampm })); })),
            dateRange.map(function (date) { return (React.createElement(reactNative.View, { style: styles$1.dayContainer, key: date.toString() },
                ...
                dayJsConvertedEvents
                    .filter(function (_a) {
                    var start = _a.start;
                    return start.isAfter(date.startOf('day')) && start.isBefore(date.endOf('day'));
                })
                    .map(function (event) { return (React.createElement(CalendarEvent, { key: "" + event.start + event.title, event: event, onPressEvent: onPressEvent, eventCellStyle: eventCellStyle, showTime: showTime, eventCount: getCountOfEventsAtEvent(event, dayJsConvertedEvents), eventOrder: getOrderOfEvent(event, dayJsConvertedEvents), overlapOffset: overlapOffset })); }),
                isToday(date) && !hideNowIndicator && (React.createElement(reactNative.View, { style: [styles$1.nowIndicator, { top: getRelativeTopInDay(now) + "%" }] })))); }))));
});
...
var CalendarHeader = React.memo(function (_a) {
    ...
    return (React.createElement(reactNative.View, { style: [isRTL ? styles$2.containerRTL : styles$2.container, style] },
...
var Calendar = React.memo(function (_a) {
    ...
    var _m = React.useState(dayjs__default['default'](date)), targetDate = _m[0], setTargetDate = _m[1];
    React.useEffect(function () {
        if (date) {
    ...
    return (React.createElement(React.Fragment, null,
        ...
        React.createElement(CalendarBody, __assign({}, commonProps, { containerHeight: height, dayJsConvertedEvents: daytimeEvents, eventCellStyle: eventCellStyle, hideNowIndicator: hideNowIndicator, overlapOffset: overlapOffset, scrollOffsetMinutes: scrollOffsetMinutes, ampm: ampm, showTime: showTime, onPressCell: onPressCell, onPressEvent: onPressEvent, onSwipeHorizontal: onSwipeHorizontal }))));
});
